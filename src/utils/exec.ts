...
/**
 * Execute a command and return only stdout if successful, or throw if it fails.
 * Useful for simple command execution where you only care about the output.
 *
 * @example
 * ```typescript
 * try {
 *   const version = await exec$Text`git --version`;
 ...
 * ```
 */
export async function exec$Text(pieces: TemplateStringsArray, ...args: any[]): Promise<string> {
    const result = await execWithOptions$({ throwOnError: true }, pieces, ...args);
    return result.stdout;
}

/**
 * Helper function to execute a simple string command with cwd option
 */
export async function execSimple(command: string, cwd?: string): Promise<ExecResult> {
    try {
        let cmd = $`cmd /c ${command}`;
